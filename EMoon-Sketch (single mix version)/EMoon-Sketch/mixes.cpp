...
// >= 512
int masterLightshow(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    
    int interval = duration / 8;
    
    if (f ==	0	) { mix_list->addLightshow(-1, interval	,	NULL	, &	poppyTheme	); }
    if (f ==	1	) { mix_list->addLightshow(-1, interval * 2	,	NULL	, &	tetrisTheme	); }
    
    if (f ==	3	) { mix_list->addLightshow(-1, interval * 2	,	NULL	, &	masterMix	); }
    if (f ==	5	) { mix_list->addLightshow(-1, interval	,	NULL	, &	aussieMix	); }

    if (f ==	6) { mix_list->addLightshow(-1, interval	* 2,	NULL	, &	masterMix	); }
...
int	messfallMix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall1	); }
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall2	); }
    if (f ==	1	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall3	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall4	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall5	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall6	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall7	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	3	,	pod0	, &	messfall8	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	3	,	pod0	, &	messfall9	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	messfall10	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	messfall11	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	5	,	pod0	, &	messfall12	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	messfall13	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	3	,	pod0	, &	messfall14	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	3	,	pod0	, &	messfall15	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall16	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall17	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall18	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall19	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall20	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall21	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall22	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	3	,	pod0	, &	messfall23	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	messfall24	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	3	,	pod0	, &	messfall25	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall26	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall27	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall28	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall29	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall30	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall31	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall32	); }
    if (f ==	16	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall33	); }
    if (f ==	16	) { APL_list->addLightshow(-1, interval*	3	,	pod0	, &	messfall34	); }
    if (f ==	17	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall35	); }
    if (f ==	17	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	messfall36	); }
    if (f ==	18	) { APL_list->addLightshow(-1, interval*	5	,	pod0	, &	messfall37	); }
    if (f ==	18	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	messfall38	); }
    if (f ==	19	) { APL_list->addLightshow(-1, interval*	3	,	pod0	, &	messfall39	); }
    if (f ==	19	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall40	); }
    if (f ==	20	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall41	); }
    if (f ==	20	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall42	); }
    if (f ==	21	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall43	); }
    if (f ==	21	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall44	); }
    if (f ==	22	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall45	); }
    if (f ==	22	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall46	); }
    if (f ==	23	) { APL_list->addLightshow(-1, interval*	3	,	pod0	, &	messfall47	); }
    if (f ==	23	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	messfall48	); }
    if (f ==	24	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall49	); }
    if (f ==	24	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall50	); }
    if (f ==	25	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall51	); }
    if (f ==	25	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall52	); }
    if (f ==	26	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	messfall53	); }
    if (f ==	26	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	messfall54	); }
    if (f ==	27	) { APL_list->addLightshow(-1, interval*	3	,	pod0	, &	messfall55	); }
    if (f ==	27	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall56	); }
    if (f ==	28	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall57	); }
    if (f ==	28	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall58	); }
    if (f ==	29	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall59	); }
    if (f ==	29	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall60	); }
    if (f ==	30	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall61	); }
    if (f ==	30	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall62	); }
    if (f ==	31	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	messfall63	); }
    if (f ==	31	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	messfall64	); }
    return interval;
...
int	tetfall1_Mix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	tetfall1_1	); }
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	tetfall1_2	); }
    if (f ==	1	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	tetfall1_3	); }
    if (f ==	1	) { APL_list->addLightshow(-1, interval*	4	,	pod0	, &	tetfall1_4	); }
...
int	flasher_Mix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_1	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_3	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_4	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_5	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_6	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_7	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_8	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_9	); }
    if (f ==	17	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_10	); }
    if (f ==	18	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_11	); }
    if (f ==	24	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_12	); }
    if (f ==	26	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_13	); }
    if (f ==	28	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	flasher_14	); }
    return interval;
...
int	spiral2_Mix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    int interval = duration / 32;

    
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_0	); }
    if (f ==	1	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_1	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_3	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_4	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_5	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_6	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_7	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_8	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_9	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_10	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_11	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_12	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_13	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_14	); }
    if (f ==	16	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_15	); }
    if (f ==	17	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_16	); }
    if (f ==	18	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_17	); }
    if (f ==	19	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_18	); }
    if (f ==	20	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_19	); }
    if (f ==	21	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_20	); }
    if (f ==	22	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_21	); }
    if (f ==	23	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_22	); }
    if (f ==	24	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_23	); }
    if (f ==	25	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_24	); }
    if (f ==	26	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_25	); }
    if (f ==	27	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_26	); }
    if (f ==	28	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_27	); }
    if (f ==	29	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_28	); }
    if (f ==	30	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_29	); }
    if (f ==	31	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_30	); }
    if (f ==	31	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_31	); }
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_32	); }
    if (f ==	1	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_33	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_34	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_35	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_36	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_37	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_38	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_39	); }
    
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_41	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_42	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_43	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_44	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_45	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_46	); }
    if (f ==	16	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_47	); }
    
    if (f ==	18	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_49	); }
    if (f ==	19	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_50	); }
    if (f ==	20	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_51	); }
    if (f ==	21	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_52	); }
    if (f ==	22	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_53	); }
    if (f ==	23	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_54	); }
    if (f ==	24	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_55	); }
    
    if (f ==	26	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_57	); }
    if (f ==	27	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_58	); }
    if (f ==	28	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_59	); }
    if (f ==	29	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_60	); }
    if (f ==	30	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_61	); }
    if (f ==	31	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_62	); }
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_64	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_65	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_66	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_67	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_68	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_69	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_70	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_71	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_72	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_73	); }
    if (f ==	16	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_74	); }
    if (f ==	19	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_75	); }
    if (f ==	20	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_76	); }
    if (f ==	21	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_77	); }
    if (f ==	22	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_78	); }
    if (f ==	24	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_79	); }
    if (f ==	27	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_80	); }
    if (f ==	28	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_81	); }
    if (f ==	29	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_82	); }
    if (f ==	30	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_83	); }
    
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_85	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_86	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_87	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_88	); }
    
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_90	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_91	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_92	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_93	); }
    
    if (f ==	19	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_95	); }
    if (f ==	20	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_96	); }
    if (f ==	21	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_97	); }
    if (f ==	22	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_98	); }
    
    if (f ==	27	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_100	); }
    if (f ==	28	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_101	); }
    if (f ==	29	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_102	); }
    if (f ==	30	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_103	); }
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_104	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_105	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_106	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_107	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_108	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_109	); }
    if (f ==	16	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_110	); }
    if (f ==	20	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_111	); }
    if (f ==	21	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_112	); }
    if (f ==	24	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_113	); }
    if (f ==	28	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_114	); }
    if (f ==	29	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_115	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_116	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_117	); }
    if (f ==	20	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_118	); }
    if (f ==	28	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral2_119	); }
    return interval;
...
int	spiral1_Mix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    int interval = duration / 16;
    
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_0	); }
    if (f ==	1	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_1	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_3	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_4	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_5	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_6	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_7	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_8	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_9	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_10	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_11	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_12	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_13	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_14	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral1_15	); }
...
int	spiral0_Mix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    int interval = duration / 16;
    if (f == 0) {
    ...
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	1	,	edge7	, &	spiral0_0	); }
    if (f ==	1	) { APL_list->addLightshow(-1, interval*	1	,	edge8	, &	spiral0_1	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	1	,	edge6	, &	spiral0_2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	1	,	pod12	, &	spiral0_3	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	1	,	pod13	, &	spiral0_4	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	1	,	pod14	, &	spiral0_5	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	1	,	pod15	, &	spiral0_6	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	1	,	edge9	, &	spiral0_7	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	1	,	edge5	, &	spiral0_8	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	1	,	pod8	, &	spiral0_9	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	1	,	pod9	, &	spiral0_10	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	1	,	pod10	, &	spiral0_11	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	1	,	pod11	, &	spiral0_12	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	1	,	edge4	, &	spiral0_13	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	edge10	, &	spiral0_14	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod4	, &	spiral0_15	); }
...
// use 512
int	spiralMix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    int interval = duration / 16;
    if (f == 0) {
        APL_list->addLightshow(p, interval, NULL, &wait);
        APL_list->addLightshow(p, interval, NULL, &wait);
    ...
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	1	,	edge7	, &	spiral0	); }
    if (f ==	1	) { APL_list->addLightshow(-1, interval*	1	,	edge8	, &	spiral1	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	1	,	edge6	, &	spiral2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	1	,	pod12	, &	spiral3	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	1	,	pod13	, &	spiral4	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	1	,	pod14	, &	spiral5	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	1	,	pod15	, &	spiral6	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	1	,	edge9	, &	spiral7	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	1	,	edge5	, &	spiral8	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	1	,	pod8	, &	spiral9	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	1	,	pod9	, &	spiral10	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	1	,	pod10	, &	spiral11	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	1	,	pod11	, &	spiral12	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	1	,	edge4	, &	spiral13	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	edge10	, &	spiral14	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod4	, &	spiral15	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	spiral15	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod1	, &	spiral15	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod2	, &	spiral15	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod3	, &	spiral15	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod5	, &	spiral15	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod6	, &	spiral15	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod7	, &	spiral15	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	edge3	, &	spiral2	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	edge11	, &	spiral2	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	edge0	, &	spiral2	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	edge1	, &	spiral2	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	edge2	, &	spiral2	); }
...
int	sparkriseMix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	10	,	edge7	, &	sparkrise0	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	8	,	edge8	, &	sparkrise1	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	8	,	edge6	, &	sparkrise2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	7	,	pod12	, &	sparkrise0	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	7	,	pod13	, &	sparkrise1	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	7	,	pod14	, &	sparkrise2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	7	,	pod15	, &	sparkrise0	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	6	,	edge9	, &	sparkrise1	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	6	,	edge5	, &	sparkrise0	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	5	,	pod8	, &	sparkrise1	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	5	,	pod9	, &	sparkrise2	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	5	,	pod10	, &	sparkrise0	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	5	,	pod11	, &	sparkrise0	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	4	,	edge4	, &	sparkrise1	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	4	,	edge10	, &	sparkrise2	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	3	,	pod4	, &	sparkrise0	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	3	,	pod5	, &	sparkrise0	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	3	,	pod6	, &	sparkrise1	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	3	,	pod7	, &	sparkrise0	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	2	,	edge3	, &	sparkrise1	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	2	,	edge11	, &	sparkrise2	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	sparkrise0	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	1	,	pod1	, &	sparkrise1	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	1	,	pod2	, &	sparkrise0	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	1	,	pod3	, &	sparkrise1	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	1	,	edge0	, &	sparkrise2	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	1	,	edge2	, &	sparkrise0	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	1	,	edge1	, &	sparkrise1	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	4	,	edge7	, &	sparkrise28	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	4	,	edge8	, &	sparkrise29	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	4	,	edge6	, &	sparkrise30	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	3	,	pod12	, &	sparkrise28	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	4	,	pod13	, &	sparkrise29	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	3	,	pod14	, &	sparkrise30	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	2	,	pod15	, &	sparkrise28	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	2	,	edge9	, &	sparkrise29	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	2	,	edge5	, &	sparkrise30	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	3	,	pod8	, &	sparkrise28	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	2	,	pod9	, &	sparkrise29	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	2	,	pod10	, &	sparkrise30	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	3	,	pod11	, &	sparkrise28	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	3	,	edge4	, &	sparkrise29	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	3	,	edge10	, &	sparkrise30	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	2	,	pod4	, &	sparkrise28	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	2	,	pod5	, &	sparkrise29	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	2	,	pod6	, &	sparkrise30	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	2	,	pod7	, &	sparkrise30	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	2	,	edge3	, &	sparkrise28	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	2	,	edge11	, &	sparkrise29	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	2	,	pod0	, &	sparkrise30	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	2	,	pod1	, &	sparkrise29	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	pod2	, &	sparkrise29	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	pod3	, &	sparkrise30	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	edge0	, &	sparkrise28	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	edge2	, &	sparkrise29	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	1	,	edge1	, &	sparkrise30	); }
...
int	redriseMix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	14	,	edge7	, &	redrise0	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	12	,	edge8	, &	redrise1	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	12	,	edge6	, &	redrise2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	11	,	pod12	, &	redrise0	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	11	,	pod13	, &	redrise2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	11	,	pod14	, &	redrise2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	11	,	pod15	, &	redrise0	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	10	,	edge9	, &	redrise1	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	10	,	edge5	, &	redrise1	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	9	,	pod8	, &	redrise0	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	9	,	pod9	, &	redrise2	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	9	,	pod10	, &	redrise1	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	9	,	pod11	, &	redrise2	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	8	,	edge4	, &	redrise1	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	8	,	edge10	, &	redrise1	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	7	,	pod4	, &	redrise0	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	7	,	pod5	, &	redrise2	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	7	,	pod6	, &	redrise1	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	7	,	pod7	, &	redrise0	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	6	,	edge3	, &	redrise1	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	6	,	edge11	, &	redrise1	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	5	,	pod0	, &	redrise0	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	5	,	pod1	, &	redrise1	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	5	,	pod2	, &	redrise0	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	5	,	pod3	, &	redrise2	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	4	,	edge0	, &	redrise1	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	4	,	edge2	, &	redrise1	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	3	,	edge1	, &	redrise1	); }
    return interval;
...
// use 512
int	riseMix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    int interval = duration / 16;
    if (f == 0) {
        APL_list->addLightshow(p, interval*1, NULL, &wait);
        APL_list->addLightshow(p, interval*2, NULL, &wait);
    ...
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	16	,	edge7	, &	rise0	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	14	,	edge8	, &	rise1	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	14	,	edge6	, &	rise2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	13	,	pod12	, &	rise0	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	13	,	pod13	, &	rise2	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	13	,	pod14	, &	rise1	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	13	,	pod15	, &	rise0	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	12	,	edge9	, &	rise2	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	12	,	edge5	, &	rise0	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	11	,	pod8	, &	rise0	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	11	,	pod9	, &	rise2	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	11	,	pod10	, &	rise1	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	11	,	pod11	, &	rise0	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	10	,	edge4	, &	rise2	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	10	,	edge10	, &	rise1	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	9	,	pod4	, &	rise0	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	9	,	pod5	, &	rise2	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	9	,	pod6	, &	rise1	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	9	,	pod7	, &	rise0	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	8	,	edge3	, &	rise2	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	8	,	edge11	, &	rise0	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	7	,	pod0	, &	rise0	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	7	,	pod1	, &	rise2	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	7	,	pod2	, &	rise1	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	7	,	pod3	, &	rise0	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	6	,	edge0	, &	rise2	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	6	,	edge2	, &	rise0	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	5	,	edge1	, &	rise1	); }
...
// use 128 to 412
int	windMix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    int interval = duration / 16;
    if (f == 0) {
        APL_list->addLightshow(p, interval*1, NULL, &wait);
        APL_list->addLightshow(p, interval*2, NULL, &wait);
    ...
    if (f ==	0	) { APL_list->addLightshow(-1, interval*	16	,	edge4	, &	wind0	); }
    if (f ==	1	) { APL_list->addLightshow(-1, interval*	15	,	edge5	, &	wind0	); }
    if (f ==	1	) { APL_list->addLightshow(-1, interval*	15	,	pod15	, &	wind0	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	9	,	edge6	, &	wind0	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	14	,	pod14	, &	wind0	); }
    if (f ==	2	) { APL_list->addLightshow(-1, interval*	13	,	pod11	, &	wind0	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	9	,	edge7	, &	wind0	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	12	,	pod13	, &	wind0	); }
    if (f ==	3	) { APL_list->addLightshow(-1, interval*	12	,	pod10	, &	wind0	); }
    if (f ==	4	) { APL_list->addLightshow(-1, interval*	11	,	pod7	, &	wind0	); }
    if (f ==	5	) { APL_list->addLightshow(-1, interval*	10	,	pod12	, &	wind0	); }
    if (f ==	6	) { APL_list->addLightshow(-1, interval*	9	,	pod9	, &	wind0	); }
    if (f ==	7	) { APL_list->addLightshow(-1, interval*	8	,	edge3	, &	wind0	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	6	,	pod3	, &	wind0	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	7	,	pod2	, &	wind0	); }
    if (f ==	8	) { APL_list->addLightshow(-1, interval*	8	,	pod8	, &	wind0	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	6	,	pod4	, &	wind0	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	4	,	edge8	, &	wind0	); }
    if (f ==	9	) { APL_list->addLightshow(-1, interval*	6	,	edge9	, &	wind0	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	4	,	edge10	, &	wind0	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	6	,	edge11	, &	wind0	); }
    if (f ==	10	) { APL_list->addLightshow(-1, interval*	6	,	pod6	, &	wind0	); }
    if (f ==	11	) { APL_list->addLightshow(-1, interval*	5	,	pod5	, &	wind0	); }
    if (f ==	12	) { APL_list->addLightshow(-1, interval*	4	,	edge2	, &	wind0	); }
    if (f ==	13	) { APL_list->addLightshow(-1, interval*	3	,	edge1	, &	wind0	); }
    if (f ==	14	) { APL_list->addLightshow(-1, interval*	2	,	pod1	, &	wind0	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	pod0	, &	wind0	); }
    if (f ==	15	) { APL_list->addLightshow(-1, interval*	1	,	edge0	, &	wind0	); }
