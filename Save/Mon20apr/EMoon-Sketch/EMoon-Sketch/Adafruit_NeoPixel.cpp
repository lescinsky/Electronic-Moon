...
Adafruit_NeoPixel::Adafruit_NeoPixel(uint16_t n, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n * 3), pin((p & DEMUX_BIT)? NETWORK_PIN_NUMBER : p), pixels(NULL)
  ,type(t), brightness(0), endTime(0), actualPin(p)
#ifdef __AVR__
  ,port(portOutputRegister(digitalPinToPort(NETWORK_PIN_NUMBER))),
   pinMask(digitalPinToBitMask(NETWORK_PIN_NUMBER)),
#endif
{
...
void Adafruit_NeoPixel::show(void) {
  ...
  if(!pixels) return;
    ...
    if (actualPin & DEMUX_BIT) {
    
    digitalWrite(DEMUX_1_INPUT, (this->actualPin & 1) ? HIGH : LOW);
    digitalWrite(DEMUX_2_INPUT, (this->actualPin & 2) ? HIGH : LOW);
    digitalWrite(DEMUX_4_INPUT, (this->actualPin & 4) ? HIGH : LOW);
    digitalWrite(DEMUX_8_INPUT, (this->actualPin & 8) ? HIGH : LOW);

//  shorten this delay!!! Just do another digitalWrite,that should be enough
    delay(1);
            digitalWrite(DEMUX_G1_INPUT, LOW);
            digitalWrite(DEMUX_G2_INPUT, LOW);
