...
void tetrisTheme(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f/interval ==	0	) { mix_list->addLightshow(-1, interval*	1	,	NULL	, &	tetrisOpener	); }
    if (f/interval ==	1	) { mix_list->addLightshow(-1, interval*	3	,	NULL	, &	tetrisMedley	); }
//    if (f/interval ==	1	) { mix_list->addLightshow(-1, interval*	3	,	NULL	, &	tetrisBorder0	); }
//    if (f/interval ==	4	) { mix_list->addLightshow(-1, interval*	1	,	NULL	, &	messfallMix	); }
//    if (f/interval ==	5	) { mix_list->addLightshow(-1, 128	,	NULL	, &	flasher_Mix	); }
//    if (f/interval ==	6	) { mix_list->addLightshow(-1, interval*	1	,	NULL	, &	mungeMix	); }
...
void redblueFramePop(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f/interval == 0) {
     ...
     if (f/interval ==	0	) { mix_list->addLightshow(-1, interval*14	,	NULL	, &	myMix	); }
    if (f/interval ==	14	) { addtoAllEdges(interval*2, &borderspeck0);}

    
    if (f/interval ==	0	) { addtoAllQuads(32, &popRedBlue);}
    if (f/interval ==	4	) { addtoAllQuads(32, &popBlueRed);}
    if (f/interval ==	8	) { addtoAllQuads(32, &popRedBlue);}
    if (f/interval ==	10	) { addtoAllQuads(16, &popBlueRedBlue);}
    if (f/interval ==	13	) { addtoAllQuads(16, &popBlueRed);}
    if (f/interval ==	15	) { addtoAllQuads(16, &popRedBlueRed);}
}
...
void redblueFrameWipe(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f/interval ==	0	) { addtoAllPods(interval*6, &redsquare0);}
    if (f/interval ==	6	) { addtoAllQuads(interval*6, &bluesquare0);}
    if (f/interval ==	12	) { addtoAllQuads(interval*4, &redsquare0);}
    
    if (f/interval ==	6	) { addtoAllEdges(interval, &borderspeck1);}
    if (f/interval ==	7	) { addtoAllEdges(interval, &h6);}
    if (f/interval ==	8	) { addtoAllEdges(interval, &h1);}
    if (f/interval ==	9	) { addtoAllEdges(interval, &aus0);}
    if (f/interval ==	10	) { addtoAllEdges(interval, &aus7);}
    if (f/interval ==	11	) { addtoAllEdges(interval, &h6);}
    if (f/interval ==	12	) { addtoAllEdges(interval, &c3);}
    if (f/interval ==	13	) { addtoAllEdges(interval, &c6);}
    if (f/interval ==	14	) { addtoAllEdges(interval, &h1);}
    if (f/interval ==	15	) { addtoAllEdges(interval, &borderspeck1);}
}
...
void tetrisOpener(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f/interval ==	0	) { mix_list->addLightshow(-1, interval	,	NULL	, &	redblueFrameWipe	); }
    if (f/interval ==	1	) { mix_list->addLightshow(-1, interval	,	NULL	, &	redblueFramePop	); }
...
void tetrisBorder0(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f/interval ==	0	) { addtoAllEdges(interval*1, &borderspeck1);}
    if (f/interval ==	1	) { addtoAllEdges(interval*15, &tetriswipe);}

    if (f/interval ==	16	) { addtoInnerEdges(interval, &	tetriswipe	); addtoOuterEdges(interval, &	g4	);}
    if (f/interval ==	17	) { addtoInnerEdges(interval, &	g4	); addtoOuterEdges(interval, &	c4	);}
    if (f/interval ==	18	) { addtoInnerEdges(interval, &	c4	); addtoOuterEdges(interval, &	g5	);}
    if (f/interval ==	19	) { addtoInnerEdges(interval, &	g5	); addtoOuterEdges(interval, &	c2	);}
    if (f/interval ==	20	) { addtoInnerEdges(interval, &	c2	); addtoOuterEdges(interval, &	c1	);}
    if (f/interval ==	21	) { addtoInnerEdges(interval, &	c1	); addtoOuterEdges(interval, &	edgewipe11	);}
    if (f/interval ==	22	) { addtoInnerEdges(interval, &	edgewipe11	); addtoOuterEdges(interval, &	e1	);}
    if (f/interval ==	23	) { addtoInnerEdges(interval, &	e1	); addtoOuterEdges(interval, &	edgewipe30	);}
    if (f/interval ==	24	) { addtoInnerEdges(interval, &	edgewipe30	); addtoOuterEdges(interval, &	aus2	);}
    if (f/interval ==	25	) { addtoInnerEdges(interval, &	aus2	); addtoOuterEdges(interval, &	aus6	);}
    if (f/interval ==	26	) { addtoInnerEdges(interval, &	aus6	); addtoOuterEdges(interval, &	aus1	);}
    if (f/interval ==	27	) { addtoInnerEdges(interval, &	tetriswipe	); addtoOuterEdges(interval, &	edgewipe12	);}
    if (f/interval ==	28	) { addtoInnerEdges(interval, &	edgewipe12	); addtoOuterEdges(interval, &	b7	);}
    if (f/interval ==	29	) { addtoInnerEdges(interval, &	b7	); addtoOuterEdges(interval, &	aus0	);}
    if (f/interval ==	30	) { addtoInnerEdges(interval, &	aus0	); addtoOuterEdges(interval, &	f0	);}
    if (f/interval ==	31	) { addtoInnerEdges(interval, &	f0	); addtoOuterEdges(interval, &	b5	);}
}
...
void tetrisMedley(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f/interval ==	0	) { mix_list->addLightshow(-1, interval*	3	,	NULL	, &	tetfall0_Mix	); }
    if (f/interval ==	3	) { mix_list->addLightshow(-1, interval*	2	,	NULL	, &	tetfall0_Mix	); }
    if (f/interval ==	5	) { mix_list->addLightshow(-1, interval*	1	,	NULL	, &	tetfall0_Mix	); }
    if (f/interval ==	6	) { mix_list->addLightshow(-1, interval*	2	,	NULL	, &	tetfall1_Mix	); }
...
void	myMix	(eMoonFrame f, eMoonFrame duration, Adafruit_NeoPixel* strip, int p) {
    ...
    if (f/interval ==	0	) { APL_list->addLightshow(-1, interval*	1	,	edge0	, &	my28	); }
    if (f/interval ==	0	) { APL_list->addLightshow(-1, interval*	1	,	edge1	, &	my28	); }
    if (f/interval ==	0	) { APL_list->addLightshow(-1, interval*	1	,	edge2	, &	my30	); }
    if (f/interval ==	1	) { APL_list->addLightshow(-1, interval*	1	,	edge3	, &	my28	); }
    if (f/interval ==	1	) { APL_list->addLightshow(-1, interval*	1	,	edge4	, &	my28	); }
    if (f/interval ==	1	) { APL_list->addLightshow(-1, interval*	1	,	edge5	, &	my30	); }
    if (f/interval ==	2	) { APL_list->addLightshow(-1, interval*	1	,	edge6	, &	my28	); }
    if (f/interval ==	2	) { APL_list->addLightshow(-1, interval*	1	,	edge7	, &	my28	); }
    if (f/interval ==	2	) { APL_list->addLightshow(-1, interval*	1	,	edge8	, &	my30	); }
    if (f/interval ==	3	) { APL_list->addLightshow(-1, interval*	1	,	edge9	, &	my28	); }
    if (f/interval ==	3	) { APL_list->addLightshow(-1, interval*	1	,	edge10	, &	my28	); }
    if (f/interval ==	3	) { APL_list->addLightshow(-1, interval*	1	,	edge11	, &	my30	); }
}
